...
- **设备管理 (`/devices`)**: 提供设备列表和详情页，支持状态查看、名称编辑、快捷操作（亮/息屏、**自动/最低亮度**）、应用列表查看、远程卸载等。
...
- **原子操作编辑器**:
  ...
  - **动作定义区**: 通过**拖拽**方式，编排一系列丰富的响应动作。动作选择器已升级为**级联菜单**（UI交互/流程控制/设备控制/断言），支持搜索，提升了选择效率。涵盖**UI交互**、**流程控制**（固定/动态等待、`wait_for_vanish`、焦点守护、结束用例）、**数据交互**（上报提取值）、**设备控制**（含亮度调节）以及**断言**。
  - 支持配置原子操作的**优先级 (Priority)**，数值越小，优先级越高。
  - **动作定义区**: 通过**拖拽**方式，编排一系列丰富的响应动作，涵盖**UI交互**、**流程控制**（固定/动态等待、`wait_for_vanish`、焦点守护、结束用例）、**数据交互**（上报提取值）、**设备控制**以及**断言** (`assert_text_equals`, `assert_element_count`)。已废弃冗余的 `assert_element_exists` 动作。
  - **循环控制**: 支持配置“最大执行次数”，允许一个原子操作在满足条件时被重复触发执行。
  - **流程穿透**: 支持配置“匹配成功后继续扫描”开关，允许将多个原子操作串联成一个不间断的执行序列。
  - **强大的主匹配器配置**:
       - **属性匹配**: 为选择器新增了 `Checked`, `Enabled`, `Selected` 三种状态的复选框，支持三态（不关心/必须为真/必须为假）匹配。
...
- **通用动作序列编辑器 (`ActionSequenceEditor.vue`)**:
  ...
  - **强制重置**: 设备详情页的“重置”按钮逻辑已升级。点击后，无论当前是否追踪到任务ID，都会向设备发送 `FORCE_RESET` 指令，无条件中止任何正在运行的即时调试任务 (Ad-hoc Task)。
  - **亮度控制**: 快捷操作区新增了“自动亮度”和“最低亮度”按钮。最低亮度模式下，屏幕亮度被设为 1（非息屏），以在保持屏幕开启和可交互的同时最大程度降低功耗和发热。
